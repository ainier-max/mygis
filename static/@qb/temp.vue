<template>    <!--class="mapClass"-->
  <div :id="id" ref="mapComponent"></div>    <!-- export default { --></template>
<script>    module.exports = {
  ...
  data: function () {
    return {map: null, outPolylineData: null, inPolylineData: null, arrInfo: [], geoMapData: "", geoMapTileLayer: null}
  },
  methods: {
    loadMap() {
    ...
    /**             * Leaflet地图加载             */            initMap(mapJson) {
      ...
      if (typeof (mapJson.crs) != "undefined") {
        if (typeof (mapJson.crs.bounds) != "undefined") {
          crsTemp = new L.Proj.CRS(mapJson.crs.epsg, mapJson.crs.proj, {
      ...
      mapOption.attributionControl = false;
      if (typeof (mapConfig.zoomSnap) != "undefined") {
        mapOption.zoomSnap = mapConfig.zoomSnap;
        mapOption.zoomDelta = mapConfig.zoomSnap;
      }
      if (crsTemp != null) {
      ...
      var tileLayerOption = {};                /**                 * 百度坐标系需要加上tms参数,高德,PGIS则不需要                 */
    ...
    /**             * 添加地图图层             */            addTileLayer(url, option) {
      var tileLayer = L.tileLayer(url, option).addTo(this.map);
      return tileLayer;
    },
    /**             * 删除地图图层             */            removeTileLayer(tileLayer) {
      this.map.removeLayer(tileLayer);
    },
    /**             * 添加缩放控件             */            addZoomControl: function (position) {
      var zoomControl = L.control.zoom({position: position});
      zoomControl.addTo(this.map);
      zoomControl.name = "缩放控件";
      return zoomControl;
    },
    /**             * 添加平移控件             */            addPanControl: function (position) {
      var panControl = L.control.pan({position: position});
      panControl.addTo(this.map);
      panControl.name = "平移控件";
      return panControl;
    },
    /**             * 移除缩放控件             */            removeZoomControl: function (object) {
      object.remove();
    },
    /**             * 移除平移控件             */            removePanControl: function (object) {
      object.remove();
    },
    /**             * 添加比例尺控件             */            addScaleControl: function (position) {
      var scaleControl = L.control.scale({position: position, imperial: false});
      scaleControl.addTo(this.map);
      scaleControl.name = "比例尺控件";
      return scaleControl;
    },
    /**             * 移除比例尺控件             */            removeScaleControl: function (object) {
      object.remove();
    },
    /**             * 地图添加点             */            addMarker: function (markerJSON) {
      var iconJson = {};
      iconJson.option = {};
      iconJson.option.iconUrl = markerJSON.iconUrl;
      iconJson.option.iconAnchor = markerJSON.iconAnchor;
    ...
    /**             * 画div方法（json格式）             * divJson.point坐标串数据             * divJson.color线的颜色             * divJson.content气泡框内容             */            addDiv(divJson) {
    ...
    /**             * 点，线，面绑定气泡框             */            bindPopup: function (object, option) {
      object.bindPopup(object.html, option);
    },
    /**             * 地图添加气泡框             */            addPopup: function (option, point, html) {
      var popup = L.popup(option).setLatLng([point[0], point[1]]).setContent(html).openOn(this.map);
      return popup;
    },
    /**             *  中心点居中             */            panTo(point) {
      this.map.panTo(point);
    },
    /**             *  级别缩放             */            zoomTo: function (level) {
      this.map.setZoom(level);
    },
    /**             * 缩放并居中             */            setView: function (point, level) {
      this.map.setView(point, level);
    },
    /**             * 删除图层             */            removeLayer: function (layer) {
      this.map.removeLayer(layer);
    },
    /**             * 删除所有气泡框             */            closeAllPopup: function () {
      this.map.closePopup();
    },
    ...
    /**             * 样式切换             */            setStyle: function (layer, option) {
      layer.setStyle(option);
    },
    /**             * 获取地图中心点             */            getMapCenter: function () {
      return this.map.getCenter();
    },
    /**             * 获取当前地图级别             */            getMapZoom: function () {
      return this.map.getZoom();
    },
    /**             * 获取地图边框             */            getMapBound: function () {
      var bound = {};
    ...
    /**             * 获取图层边框             */            getLayerBound: function (layerObject) {
      var bound = {};
    ...
    /**             * 根据图层边框居中             */            setCenterByBound: function (boundObject) {
    ...
    /**             * 绑定tip             */            bindTooltip: function (obj, content, option) {
    ...
    /**             * 聚合图方法             * clusterPoints：聚合点             * icon：聚合点图标             */            addCluster(clusterMarkers, option) {
      var ClusterLayer = L.markerClusterGroup(option);
      ClusterLayer.addLayers(clusterMarkers);
      this.map.addLayer(ClusterLayer);
      return ClusterLayer;
    },
    /**             * 创建地图图标             */            createIcon(iconJson) {
      var tempIcon = L.icon(iconJson.option);
      return tempIcon;
    },
    /**             * 创建marker，但是不进行上图             * markerJson.xy xy坐标点             * markerJson.option marker参数设置             */            createMarker(markerJson) {
      var marker = new L.marker(markerJson.xy, markerJson.option);
      return marker;
    },
    /**             * 热力图方法addHeatMap             * heatPoints：热力点数组             * option:聚合半径等参数设置             */            addHeatMap(heatPoints, option) {
      var heatmap_layer = L.heatLayer(heatPoints, option);
      this.map.addLayer(heatmap_layer);
      return heatmap_layer;
    },
    /**             * 蚂蚁线的方法             * antPathJson.xys 坐标串             * antPathJson.option 参数设置             * @param antPathJson             * @returns {*}             */            addAntPath(antPathJson) {
      var antPolyline = L.polyline.antPath(antPathJson.xys, antPathJson.option);
      antPolyline.addTo(this.map);
      return antPolyline;
    },
    /**             * 波纹点的方法             * pulseJson.xy  xy坐标             * pulseJson.option  参数设置             * @param pulseJson             */            addPulsingIcon(pulseJson) {
      var pulsingIcon = L.icon.pulse(pulseJson.option);
      var marker = L.marker(pulseJson.xy, {icon: pulsingIcon});
      marker.addTo(this.map);
      return marker;
    },
    /**             * 添加散点图的方法             * pulseJson.xy  xy坐标             * pulseJson.option  参数设置             * @param pulseJson             */            addScatter(scatterData, parts) {
      var markers = [];
    ...
    /**             * 添加蜂巢             */            addBeeHive(dataSet, options) {
      var mapvLayer = window.mapv.leafletMapLayer(dataSet, options).addTo(this.map);
      return mapvLayer;
    },
    /**             * 虚线             * dashJson.xys 坐标串             * dashJson.option 参数设置             * @param dashJson             */            addDashPolyline(dashJson) {
    ...
    /**             * 箭头线             */            addDirectionPolyline(polylineObj, option) {
      L.polylineDecorator(polylineObj, {
        patterns: [{
          offset: 20,
          repeat: 50,
          symbol: L.Symbol.arrowHead({pixelSize: option.arrowSize, pathOptions: {fillOpacity: 1, weight: 0}})
        }]
      }).addTo(this.map);
    },
    /**             * 图标线             */            addIconPolyline(iconPolylineObj) {
      L.polylineDecorator(iconPolylineObj, {
        patterns: [{
          offset: 1,
          repeat: 100,
          symbol: L.Symbol.marker({rotate: true, markerOptions: {icon: iconPolylineObj.icon}})
        }]
      }).addTo(this.map);
    },
    /**             * 平行线             */            addParallelPolyline(polylineJSON) {
    ...
    addDirectionIcon(polylineObj) {
      ...
      for (var i = 0; i < latlngs.length; i++) {
        if (i == 0) {
          ...
          var angle = Math.atan2((endPoint.lng - startPoint.lng), (endPoint.lat - startPoint.lat));
    ...
    /**             * 图标移动             * movingMarkerJson.xys : 坐标串             * movingMarkerJson.xys[i].popup : 为每个坐标创建气泡框             * movingMarkerJson.icon : 创建移动图标             * movingMarkerJson.speed : 移动速度（米/秒）             */            addMovingMarker(movingMarkerJson) {
      let the = this;
      var times = [];
      var point_popups = [];
    ...
    /**             * 动态画线             * @param points ： 坐标串点             * @param _icon ：移动图标             * @returns {*}             */            addMovingPolyline(points, _icon) {
      var featureGroup = [];
    ...
    /**             * 两点之间的距离，单位米，存在些微误差             */            distace(point1, point2) {
    ...
    /**             * 米转度             * @param meter             */            meterToDegree(meter) {
      var degree = meter / (1000 * 101.11);
      return degree;
    },
    /**             * 度转米             * @param meter             */            degreeToMeter(degree) {
      var meter = (1000 * 101.11) * degree;
      return meter;
    },
    /**             * 创建气泡框             */            createPopup(popupJson) {
      var popup = L.popup(popupJson.option).setLatLng(popupJson.xy).setContent(popupJson.content);
      return popup;
    },
    /**             * 采集点             * markerIcon:采集点图标             * dragFlag:是否可拖动             */            drawMarker: function (markerJSON, dragFlag, callback) {
      let the = this;
      this.map.on('click', function (e) {
        ...
        if (dragFlag == true) {
          gather_marker_layer = new L.marker([e.latlng.lat, e.latlng.lng], {icon: gatherIcon, draggable: true});
          ...
          gather_marker_layer = new L.marker([e.latlng.lat, e.latlng.lng], {icon: gatherIcon, draggable: false});
        }
    ...
    /**             * 采集线             */            drawPolyline: function (callback) {
      this.clearHooks();
      let the = this;
    ...
    /**             * 删除采集时最后一个点             */            drawDeleteLastVertex() {
    ...
    /**             * 采集面             */            drawPolygon: function (callback) {
      this.clearHooks();
      var the = this;
    ...
    /**             * 采集矩形             */            drawRect: function (callback) {
      this.clearHooks();
      let the = this;
    ...
    /**             * 采集圆             */            drawCircle: function (callback) {
      this.clearHooks();
      let the = this;
    ...
    /**             * 点（已在地图上）进入编辑状态             * markerObj:要编辑的点对象             */            startEditMarker(markerObj) {
      markerObj.xy = markerObj._latlng;
      markerObj.dragging.enable();
      return markerObj;
    },
    /**             * 点（已在地图上）结束编辑状态             * markerObj:要结束编辑的点对象             */            endEditMarker(markerObj) {
      markerObj.dragging.disable();
      markerObj.off('drag');
      markerObj.xy = markerObj._latlng;
      return markerObj;
    },
    /**             * 线（已在地图上）进入编辑状态             * polylineObj:要编辑的线对象             */            startEditPolyline(polylineObj) {
      var zbc = polylineObj.getLatLngs();
      polylineObj.zbc = zbc;
      polylineObj.editing.enable();
      return polylineObj;
    },
    /**             * 线（已在地图上）结束编辑状态             * polylineObj:要结束编辑的线对象             */            endEditPolyline(polylineObj) {
    ...
    /**             * 启动多边形编辑状态             * polygonObj:要编辑的多边形对象             */            startEditPolygon(polygonObj) {
      var zbc = polygonObj.getLatLngs()[0];
      polygonObj.zbc = zbc;
      polygonObj.editing.enable();
      return polygonObj;
    },
    /**             * 结束多边形编辑状态             * polygonObj:要编辑的多边形对象             */            endEditPolygon(polygonObj) {
    ...
    /**             * 矩形进入编辑状态             * rectObj:要编辑的矩形对象             */            startEditRect(rectObj) {
      rectObj.editing.enable();
      var zbc = rectObj.getLatLngs()[0];
      rectObj.zbc = zbc;
    },
    /**             * 矩形结束编辑状态             * rectObj:要结束编辑的矩形对象             */            endEditRect(rectObj) {
    ...
    /**             * 添加canvas             */            addCanvas(obj, fun) {
      var canvasLayer = L.canvasLayer().delegate(obj, fun).addTo(this.map);
      return canvasLayer;
    },
    /**             * 地图上的canvas刷新             */            canvasRefresh() {
      var centerPoint = this.map.getCenter();
      var lat = centerPoint.lat;
      var lng = centerPoint.lng + 0.00000001;
      this.map.setView([lat, lng]);
    },
    /**             *  添加canvas曲线             * @param ctx             * @param fromPoint             * @param toPoint             */            addCanvasCurve(ctx, fromPoint, toPoint, curveColor) {
    ...
    /**             * 添加曲线箭头             */            addCurveArrow(ctx, fromPoint, toPoint, curveColor) {
      var returnObj = this.calcalateXY(fromPoint, toPoint);
      var centerAngle = (returnObj.startAngle + returnObj.endAngle) / 2;
      var centerX = Math.cos(centerAngle) * returnObj.radius;
      var centerY = Math.sin(centerAngle) * returnObj.radius;

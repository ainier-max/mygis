<template>
  <div class="mapClass" :id="id" ref="mapComponent">
    ...
    export default {
        ...
        methods: {
            ...
            loadMapByParam(paramConfig) {
                if (typeof (window.L) != "undefined" && typeof (window.L.Proj) != "undefined" && this.id != "" && typeof (document.getElementById(this.id)) != "undefined") {
                    console.log("地图id值", this.id);
                    var mapJson = paramConfig;//该值从mapConfig.js获取
                    mapJson.id = this.id;
                    mapJson.useType = this.type; //调用使用类型[vue,js],必须指定
                    this.initMap(mapJson);
                } else {
            ...
            onClick: function (callback) {
                window.cbcMapCallback = callback;
                this.map.on("click", this.onClickEventBack);
            },
            onClickEventBack(e) {
                e.lngLat = {};
                e.lngLat.lng = e.latlng.lng;
                e.lngLat.lat = e.latlng.lat;
                window.cbcMapCallback(e);
            },
            //地图移除unload事件
            offUnload: function (callback) {
                this.map.off("unload");
            },
            //地图绑定unload事件
            onUnload: function (callback) {
                this.map.on("unload", callback);
            },
             ...
             * icon：聚合点图标
             */
            addCluster(clusterMarkers) {
                var ClusterLayer = L.markerClusterGroup({chunkedLoading: false});
            ...
            drawPolygon: function (callback) {
                ...
                this.map.on('draw:created', function (e) {
                    ...
                    the.map.off("draw:created");
                    var zbc = gather_polygon_layer.getLatLngs()[0];
            ...
            drawRect: function (callback) {
                ...
                this.map.on('draw:created', function (e) {
                    ...
                    var zbc = gather_rect_layer.getLatLngs()[0];

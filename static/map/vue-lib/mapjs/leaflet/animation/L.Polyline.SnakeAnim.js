...
//var moving_marker = L.marker([0,0],{icon: pulsingIcon});
...
L.Polyline.include({
  ...
  _speed:200,
	...
	snakeIn: function(flag,marker,speed){
    //时候居中
    this._movingToCenter=flag;
    //移动图标
    ...
    this._speed=speed;
		...
		if (!this._snakeLatLngs) {
			this._snakeLatLngs = L.Polyline._flat(this._latlngs) ?
				[ this._latlngs ] :
				this._latlngs ;
			//console.log(this._snakeLatLngs);
		}
    ...
    var forward = diff * this._speed / 1000;	// In pixels
...
// 		console.log('Vertices: ', this._latlngs);
		...
		//动画居中
		if(this._movingToCenter==true){
			this._map.panTo(headLatLng);
		}
		if(this._movingMarker!=null){
      this._movingMarker.setLatLng(headLatLng);
    }
  ...
  _speed:200,
	...
	_snakeNext: function() {
		...
		if ('snakeIn' in currentLayer) {
			...
			currentLayer.snakeIn(this._movingToCenter,this._movingMarker,this._speed);
		} else {
			setTimeout(this._snakeNext.bind(this), this.options.snakingPause);
		}
		this.fire('snake');

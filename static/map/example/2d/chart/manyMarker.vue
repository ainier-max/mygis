<template>
  <div>
    ...
    <el-button type="primary" @click='addManyMarkerTest();' style='z-index:1000;position:absolute;top:50px;left:50px'>
      大量上图
    </el-button>
  </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            addManyMarkerTest() {
                if (canvasLayer != null) {
                    this.$refs.map.removeLayer(canvasLayer);
                    canvasLayer = null;
                }
                this.$refs.map.zoomTo(8);
                canvasLayer = this.$refs.map.addCanvas(this, "onDrawLayer");
                this.$refs.map.onClick(this.mapEventCallBack);
            },
            onDrawLayer(info) {
            ...
            mapEventCallBack(e) {
                ...
                //矩形过滤数据
                var degree = this.$refs.map.meterToDegree(500);
                var xmin = lng - degree;
                var xmax = lng + degree;
                var ymin = lat - degree;
                var ymax = lat + degree;
                ...
                for (var i = 0; i < newPoints.length; i++) {
                    var distace = this.$refs.map.distace(clickPoint, newPoints[i]);
                ...
                if (typeof (recentPoint.point) != "undefined") {
                    ...
                    this.$refs.map.addPopup(option, recentPoint.point, html);
                }
            }
        },
        mounted: function () {

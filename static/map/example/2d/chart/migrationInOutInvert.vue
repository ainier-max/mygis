<template>
  <div>
    <map-component ref="map" :id="id" :type="type" :config="config" style='z-index:100;position: absolute;top: 0; bottom: 0;right: 0;left: 0;background-color: #3c3c3c;'></map-component>
    <el-button type="primary" @click='inOutMigrationTest()' style='z-index:1000;position:absolute;top:50px;left:60px'>
      双向迁徙图
    </el-button>
    ...
    export default {
        ...
        methods: {
            inOutMigrationTest() {
                ...
                //outPolylineData.push({"from":fromPoint,"to":toPoint4,"count":Math.ceil(Math.random() * 100),color:"#5a36e2"});
                outPolylineData.push({
                    "from": fromPoint,
                    "to": toPoint5,
                    "count": Math.ceil(Math.random() * 100),
                ...
                //inPolylineData.push({"from":toPoint4,"to":fromPoint,"count":Math.ceil(Math.random() * 100),color:"#E22F2D"});
                inPolylineData.push({
                    "from": toPoint5,
                    "to": fromPoint,
                    "count": Math.ceil(Math.random() * 100),
                ...
                //注意：outPolylineData与inPolylineData须大小一致，并且下标内容中的from与to相同，如count无数据，请填写0。
                this.$refs.map.inOutMigration(outPolylineData, inPolylineData);
            },
        ...
        updated: function () {
            var the = this;
            window.mapResizeErd.listenTo(document.getElementById("map"), function (element) {

<template>
  <div>
    ...
    <el-button type="primary" @click='deletePlot();'
               style='z-index:1000;position:absolute;top:200px;left:50px'>删除进攻方向(燕尾)
    </el-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    addPlot(){
      this.tailedAttackArrowLayer=this.$refs.plotRef.addTailedAttackArrow(this.gatherPosition);
    },
    editPlot(){
      this.$refs.plotRef.editTailedAttackArrow(this.tailedAttackArrowLayer,this.gatherIconOption);
    },
    endPlot(){
      this.$refs.plotRef.endTailedAttackArrow(this.tailedAttackArrowLayer,this.editCallback);
    },
    editCallback(newAssaultDirectionLayer){
      this.tailedAttackArrowLayer=newAssaultDirectionLayer;
      console.log("修改后的坐标信息为：",this.tailedAttackArrowLayer.gatherPosition);
    },
    deletePlot(){
      this.$refs.map.removeLayer(this.tailedAttackArrowLayer);
    }

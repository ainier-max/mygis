<template>
  <div>
    ...
    <el-button type="primary" @click='deletePlot();'
               style='z-index:1000;position:absolute;top:200px;left:50px'>删除聚集地
    </el-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    addPlot(){
      this.plotLayer=this.$refs.plotRef.addGatheringPlace(this.gatherPosition);
    },
    editPlot(){
      this.$refs.plotRef.editGatheringPlace(this.plotLayer,this.gatherIconOption);
    },
    endPlot(){
      this.$refs.plotRef.endGatheringPlace(this.plotLayer,this.editCallback);
    },

<template>
  <div>
    ...
    <el-button type="primary" @click='deletePlot();'
               style='z-index:1000;position:absolute;top:200px;left:50px'>删除战斗分队箭头
    </el-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    addPlot(){
      this.plotLayer=this.$refs.plotRef.addSquadCombat(this.gatherPosition);
    },
    editPlot(){
      this.$refs.plotRef.editSquadCombat(this.plotLayer,this.gatherIconOption);
    },
    endPlot(){
      this.$refs.plotRef.endSquadCombat(this.plotLayer,this.editCallback);
    },
